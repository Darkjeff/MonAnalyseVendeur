...
/**
 *   	\file       ecoute_list.php
 *		\ingroup    monanalysevendeur
 *		\brief      List page for ecoute
 */
...
require_once __DIR__.'/class/ecoute.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'ecoutelist'; // To manage different context of search
...
$hookmanager->initHooks(array('ecoutelist')); // Note that conf->hooks_modules contains array
...
$permissiontoread = $user->rights->monanalysevendeur->ecoute->read;
$permissiontoadd = $user->rights->monanalysevendeur->ecoute->write;
$permissiontodelete = $user->rights->monanalysevendeur->ecoute->delete;
...
//$help_url="EN:Module_Ecoute|FR:Module_Ecoute_FR|ES:Módulo_Ecoute";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("Ecoutes"));
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page)
{
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/monanalysevendeur/ecoute_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/monanalysevendeur/ecoute_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendEcouteRef";
$modelmail = "ecoute";
$objecttmp = new Ecoute($db);
...
foreach ($object->fields as $key => $val)
{
	...
	if (!empty($arrayfields['t.' . $key]['checked'])) {
		...
		if(strpos($key, 'fk_product_univers')===false) {
